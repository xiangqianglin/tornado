...
from utils.account import add_post,get_all_posts,get_post,get_posts_for  #数据库的图片调用
...
class IndexHandler(BaseHandler):  #首页 用户上传图片的展示
    ...
    def get(self):
        posts = get_posts_for(self.current_user)                  #在数据库查询图片出来
...
class UploadHandler(BaseHandler):                                #上传图片  保存             7
    ...
    def post(self):
        ...
        self.redirect('/post/{}'.format(post_id))                    #跳转到post.html  ========post_id找不到？？？？？？？？
